<x-frontend-layout>
    <section>
        <div class="container flex justify-center text-center py-20">
            <div>
                <h1 class="text-3xl text-slate-500">List Your Business or Store at DigiNepal Pvt. Ltd. <br>
                    Reach 1,00,000 + new customers.
                </h1>

                <!-- Modal toggle -->
                <button data-modal-target="request-modal" data-modal-toggle="request-modal" class="mt-5 bg-primary text-white px-4 py-2 rounded-md" type="button">
                    SEND REQUEST
                </button>
            </div>
                        ...
                        <form action="{{ route('vendor_request')}}" method="post">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="name" id="name" class="w-full px-2 py-1 border rounded-md" required value="{{old('name')}}">
                                </div>
                                <div>
                                    <label for="email">Email <span class="text-red-600">*</span></label>
                                    <input type="email" name="email" id="email" class="w-full px-2 py-1 border rounded-md" required value="{{old('email')}}">
                                </div>
                                <div>
                                    <label for="phone">Phone <span class="text-red-600">*</span></label>
                                    <input type="text" name="phone" id="phone" class="w-full px-2 py-1 border rounded-md" required value="{{old('phone')}}">
                                </div>
                                <div>
                                    <label for="shop_name">Shop Name <span class="text-red-600">*</span></label>
                                    <input type="text" name="shop_name" id="shop_name" class="w-full px-2 py-1 border rounded-md" required value="{{old('shop_name')}}">
                                </div>
